...
import { ref, watch, reactive, Reactive, onMounted } from "vue";
...
async function getReceiverUserInfo(userId: string) {
    const formData = new FormData();
    formData.append("user_id", userId);

    const resp = await axiosInstance
        .post("/users/search-by-id/", formData, {
            headers: {
...
<template>
    <div
        v-if="!isArchiveChatOpen && !isNewChatPageOpen"
        class="flex flex-col w-full h-full"
    >
        <div
            class="select-none flex flex-row relative h-[64px] w-full pt-4 pl-4 justify-start items-center"
        >
            <h1 class="text-[22px] font-bold">Chats</h1>
            <button
                @click="toggleNewChatPage"
            ...
            <div
                v-if="isChatOptionsDivOpen"
                class="absolute custom-shadow-inset3 top-16 right-4 w-[225px] h-auto flex flex-col items-center py-2 bg-white z-40 text-[14.5px] font-normal"
            >
                <div
                    class="w-full h-[40px] hover:bg-[#f5f6f6] pl-6 flex items-center cursor-pointer"
                >
                    <p>New group</p>
                </div>
                <div
        ...
        >
            <button
                ...
                class="w-[24px] h-[24px] ml-2 cursor-pointer"
            >
                <svg
                    viewBox="0 0 24 24"
                    height="24"
            ...
            <input
                @click="isSearchModeOn = true"
                v-model="searchText"
                type="text"
                class="w-[90%] ml-4 h-full outline-none"
                placeholder="Search"
            />
            <button
                v-if="searchText.length >= 1"
                @click="closeSearchBar"
                class="w-[24px] h-[24px] cursor-pointer mr-2"
            >
                <svg
                    viewBox="0 0 24 24"
                    height="24"
        ...
        <div class="select-none flex flex-row gap-x-2 pl-3 mt-2">
            ...
            <button
                @click="chatShowingFilter('unread')"
                :class="[
            ...
            <button
                @click="chatShowingFilter('favorites')"
                :class="[
                    currentChatFilter === 'favorites'
                        ? 'bg-[#e7fce3] text-green-600'
                        : 'bg-[#f0f2f5] text-gray-600',
                ]"
                class="w-auto px-3 h-[32px] rounded-3xl cursor-pointer"
            ...
            <button
                @click="chatShowingFilter('groups')"
                :class="[
                    currentChatFilter === 'groups'
                        ? 'bg-[#e7fce3] text-green-600'
                        : 'bg-[#f0f2f5] text-gray-600',
                ]"
                class="w-auto px-3 h-[32px] rounded-3xl cursor-pointer"
            ...
            >
                ...
                <div class="flex flex-col ml-4 w-full">
                    ...
                    <div class="relative flex flex-row items-center w-full">
                        <p class="font-normal text-[14px] text-gray-600">
                            Your last chat...
                        </p>
                        <button
                            @click="toggleChatOptions(1)"
                        ...
                        <div
                            v-if="openChatOptions.includes(1)"
                            class="absolute top-8 right-2 flex w-[198px] h-auto py-2 flex-col items-center bg-white z-[999] custom-shadow-inset cursor-default"
                        >
                            <div
                                class="w-full h-[40px] flex items-center pl-6 cursor-pointer hover:bg-[#f5f6f6]"
                            >
    ...
    <div
        v-if="isNewChatPageOpen && !isArchiveChatOpen"
        class="flex flex-col w-full h-full"
    >
